...
QUtfStringList ReadFile(QString filepath)
{
    QUtfStringList lines;
    QFile file(filepath);
...
int main(int, char **)
{
    ...
    QUtfStringList s = ReadFile( ":lines.txt");

    QDebug deb = qDebug();

    for (auto str : s) {
        for (int i = 0; i < str.size(); i++)
            deb << QString::number(str.at(i).codePoint(),16) << str.at(i).isLetter();
        deb << "\n";
    }
